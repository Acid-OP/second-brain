...
export function Dashboard() {
  ...
  const [isCopied, setIsCopied] = useState(false); // State for button text
  ...
  const handleShare = async () => {
    try {
      ...
      setIsCopied(false); // Reset button text when modal opens
    } catch (e) {
  ...
  const handleMakePrivate = async () => {
    try {
      ...
      setShareLink(null);
      setShareModalOpen(false); // Close modal if open
                ...
                <div className="flex justify-center gap-4 mt-6">
                  <Button
                    onClick={() => {
                      ...
                      setToastMessage("Copied to clipboard!"); // Show toast confirmation
                      setIsCopied(true); // Update button text to "Copied"
                    }}
                    variant="primary"
                    text={isCopied ? "Copied" : "Copy"} // Dynamic button text
